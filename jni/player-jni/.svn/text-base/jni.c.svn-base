...
JNIEXPORT int JNICALL NAME(seek)(JNIEnv *env, jobject thiz, jdouble time) {
	...
	__android_log_print(ANDROID_LOG_INFO, "chenhb",
		 				"player_seek %fd", seekTime);
    return player_seek(seekTime);
}
...
JNIEXPORT int JNICALL NAME(getCurrentTime)(JNIEnv *env, jobject thiz) {
	 int current = player_get_current_time();
	 __android_log_print(ANDROID_LOG_INFO, "chenhb",
	 				"player_get_current_time2 %d", current);
    return current;
}
...
JNIEXPORT jint JNICALL NAME(isPlaying)(JNIEnv *env, jobject thiz) {
...
JNIEXPORT jint JNICALL NAME(getAudioTimeBase)(JNIEnv *env, jobject thiz) {
    return player_get_audio_time_base();
}

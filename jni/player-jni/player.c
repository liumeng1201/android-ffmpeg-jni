...
int player_play(double start, int audio, int video, int subtitle) {
    ...
    //add by chenhb
    uint8_t* pixels;
	if (AndroidSurface_getPixels(gCtx->width, gCtx->height, &pixels) != 0) {
		return -1;
	}
	...
	// of AVPicture
	avpicture_fill((AVPicture *) gCtx->frame,
				   pixels,
				   PIX_FMT_RGB565,
				   gCtx->width,
				   gCtx->height);
...
int player_seek(double time) {
   ...
   /* if (gCtx->audio_enabled && ((int64_t)(time / gCtx->audio_time_base) != AV_NOPTS_VALUE)) {
    ...
    gCtx->pause = -1;
    err = avformat_seek_file(gCtx->av_ctx, -1, INT64_MIN, (int64_t)(time * AV_TIME_BASE), INT64_MAX, 0);
    if (err < 0) {
...
int player_get_current_time() {
	...
	if (!gCtx)
		return -1;
	if (gCtx->audio_enabled) {
		current = (int) gCtx->audio_last_pts;
		__android_log_print(ANDROID_LOG_INFO, "chenhb",
				"player_get_current_time %d", current);
		return current;
	} else if (gCtx->video_enabled) {
		current = (int) gCtx->video_last_pts;
		__android_log_print(ANDROID_LOG_INFO, "chenhb",
				"player_get_current_time %d", current);
		return current;
	} else {
...
int player_is_playing() {
    return gCtx ? gCtx->pause : 0;
}

int player_get_audio_time_base() {
	return gCtx ? gCtx->audio_time_base : 0;
}
